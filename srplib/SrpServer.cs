...
namespace SecureRemotePassword
{
	...
	public class SrpServer : ISrpServer
	{
		...
		public SrpEphemeral GenerateEphemeral(byte[] verifier)
		{
			// B = kv + g^b (b = random number)
			var b = SrpInteger.RandomInteger(Parameters.HashSizeBytes);
			var B = ComputeB(SrpInteger.FromByteArray(verifier), b);

			return new SrpEphemeral
			{
				Secret = b.ToByteArray(),
		...
		public SrpSession DeriveSession(byte[] serverSecretEphemeral, byte[] clientPublicEphemeral, byte[] salt, string username, byte[] verifier, byte[] clientSessionProof)
		{
			// N — A large safe prime (N = 2q+1, where q is prime)
			// g — A generator modulo N
			// k — Multiplier parameter (k = H(N, g) in SRP-6a, k = 3 for legacy SRP-6)
			...
			var expected = M;
			var actual = SrpInteger.FromByteArray(clientSessionProof);
			if (actual != expected)
			{
				// fixme: .code, .statusCode, etc.
