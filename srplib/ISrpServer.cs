namespace SecureRemotePassword
{
	...
	public interface ISrpServer
	{
		...
		SrpEphemeral GenerateEphemeral(byte[] verifier);
		...
		SrpSession DeriveSession(byte[] serverSecretEphemeral, byte[] clientPublicEphemeral, byte[] salt, string username, byte[] verifier, byte[] clientSessionProof);
	}
}